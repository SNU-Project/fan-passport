...
body.push(
  P("설계는 두 개의 계약으로 이루어진다. 하나는 팬이 공연에 실제로 다녀왔다는 사실을 쌓아 두는 팬 여권이고, 다른 하나는 그 기록을 근거로 좌석을 나눠 주는 티켓함이다. 둘 중 하나만으로는 문제가 풀리지 않는다. 여권만 있으면 기록의 쓸모가 굿즈 선구매 정도에 그쳐 암표와 연결되지 않고, 티켓함만 있으면 반납된 표를 누구에게 줄지 기준이 없어 결국 선착순, 즉 매크로 경쟁으로 되돌아간다. 여권의 기록을 티켓함의 배분 기준으로 쓸 때 비로소 두 문제가 함께 풀린다."),
);

body.push(H2(1, "두 개의 계약"));
body.push(
  P([T("팬 여권(FanPassport)", { bold: true }), T("은 사람 한 명당 하나씩 발급되어 평생 쓰는 계약이다. 실명 확인을 마친 사람에게만 발급되고, 어느 아티스트의 어느 공연이든 기록은 이 여권 하나에 모인다. 도장은 공연장에 실제로 입장한 순간에만 찍히므로, 표를 사 두기만 하고 가지 않으면 아무것도 남지 않는다. 그리고 이 도장은 남에게 넘길 수 없다. 전송 함수가 존재하기는 하지만, 호출하면 언제나 실패하도록 만들어 두었다.")]),
  P([T("티켓함(TicketBox)", { bold: true }), T("은 공연 한 건마다 새로 만들어져 그 공연의 좌석 배분·발권·검표를 맡는 계약이다. 좌석 수, 정가, 보증금, 공연 시각은 계약을 만들 때 정해지고 그 뒤로는 기획사조차 바꿀 수 없다. 모든 티켓함이 같은 팬 여권을 바라보기 때문에, 서로 다른 예매처에서 본 공연이라도 도장은 한 여권에 누적된다. 참여 주체는 여권을 발급하는 기관, 공연을 여는 기획사, 현장 검표 단말, 그리고 팬이다.")]),
);

body.push(H2(2, "공연 한 건이 진행되는 순서"));
body.push(
  NumItem([T("라운드 개설. ", { bold: true }), T("기획사가 좌석을 두 갈래로 나눠 응모를 연다. 예컨대 100석 중 60석은 점수로 겨루는 우선권 좌석, 40석은 점수와 무관한 추첨 좌석으로 연다.")], "flow"),
  NumItem([T("응모. ", { bold: true }), T("팬은 가진 점수 중 얼마를 걸지 정한다. 5회 참석해 500점이 있는 팬은 300점을 걸 수도 있고, 신규 팬은 0점으로 추첨에만 참여할 수도 있다. 건 점수는 응모하는 순간 차감되며 떨어져도 돌려주지 않는다. 그래야 모두가 최대치를 지르지 않고 정말 가고 싶은 공연에만 점수를 쓰게 된다. 여권이 없거나 이미 표를 가진 계정, 같은 라운드에 두 번 응모하는 계정은 거부된다.")], "flow"),
  NumItem([T("배정. ", { bold: true }), T("우선권 좌석은 건 점수가 높은 순으로, 추첨 좌석은 남은 응모자 중 무작위로 정해진다. 당첨자는 정가와 보증금을 함께 내고 표를 받는다.")], "flow"),
  NumItem([T("반납·재배분. ", { bold: true }), T("못 가게 된 사람은 정가와 보증금을 전액 돌려받고 좌석을 반납한다. 손해도 이익도 없으므로 애초에 되팔 목적으로 사 둘 이유가 사라진다. 반납된 좌석은 다음 라운드로 넘어가 같은 절차로 다시 배분된다.")], "flow"),
  NumItem([T("입장·정산. ", { bold: true }), T("검표를 통과하는 순간 여권에 도장이 찍히고 보증금이 환급된다. 반납도 입장도 하지 않은 좌석은 공연 후 보증금이 몰수되어 다음 회차 리워드 재원이 된다. 반납에는 벌칙이 없고 무단 불참에만 손해가 있으므로, 빈 좌석 대신 반납하도록 유도된다.")], "flow"),
);

body.push(H2(3, "표가 넘어가지 않는 이유"));
body.push(
  P("표를 보낼 수 있는 주소는 티켓함 자신뿐이다. 친구에게도 중고 거래 구매자에게도 보낼 수 없고 마켓플레이스 등록도 실패한다. 티켓함으로 보내는 것만 통과하는데 그것이 곧 반납이므로, 모든 이전 경로가 반납 하나로 수렴한다. 현금이 오갔더라도 표의 주인은 그대로 남아, 공연장에서 신분증과 대조하면 입장할 수 없다. 여기서 중요한 것은 구매 기록을 블록체인에 남긴다는 것이 아니라 표 자체가 블록체인 위에 있다는 점이다. 예매처 서버라면 관리자가 소유자 이름 한 줄만 고치면 그만이지만, 이 계약에는 그 한 줄을 고칠 수 있는 함수 자체가 존재하지 않는다."),
);

body.push(H2(4, "점수가 시간이 지나면 줄어드는 이유"));
body.push(
  P("도장 하나는 100점이고 730일에 걸쳐 0점으로 줄어든다. 오래전에 한두 번 다녀온 사람이 영원히 1순위를 차지하는 것을 막고 새로 유입된 팬이 따라잡을 여지를 남기기 위해서다. 구현 과정에서 확인한 함정이 하나 있다. 쌓인 점수만 줄이고 이미 쓴 점수를 그대로 두면 시간이 지날수록 차감액이 상대적으로 커져 모든 팬의 점수가 0으로 수렴해 버린다. 따라서 쓴 점수도 같은 속도로 함께 줄어들도록 계산해야 한다. 한편 누적 참석 횟수는 누구나 조회할 수 있는 공개 값이어서, 굿즈 판매처가 별도의 데이터 연동 계약 없이 '5회 이상 참석' 같은 조건을 직접 확인할 수 있다."),
);
...
body.push(H1(6, "결론"));
body.push(
  P("암표 방지 실패와 팬 이력의 파편화는 '양도 불가능한 참여 이력을 배분의 화폐로 쓴다'는 하나의 장치로 함께 해소된다. 표 부족분의 이득을 암표상이 아닌 참여 팬에게 귀속시키는 것이 현실적 목표이며, 스마트컨트랙트로 구현해 22개 항목으로 검증했다(부록 A·B). 프라이버시 보호와 검증가능 난수 생성이 후속 과제다."),
);
