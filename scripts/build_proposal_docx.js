...
body.push(
  table(
    ...
    [
      ...
      ["TicketBox", "공연별 좌석·가격·티켓 상태 관리", "공식 경로만 가능"],
...
body.push(
  P("리워드는 현금이나 가상자산으로 교환할 수 없고 타인에게 전송할 수도 없는 팬 활동 점수이므로, 되팔아 차익을 얻는 구조와 분리된다. 실제 입장 후에만 확정되어 표를 대량 확보해도 관람하지 않으면 쌓이지 않는다. 확정된 점수는 730일에 걸쳐 0으로 줄어드는데, 오래전에 다녀온 사람이 영원히 우선순위를 차지하지 않도록 하기 위해서다. 한편 누적 참석 횟수는 누구나 조회할 수 있어, 굿즈 판매처가 별도 연동 없이 등급 조건을 직접 확인할 수 있다."),
);
...
body.push(
  ...
  P([T("TicketBox.sol  ", { bold: true }), T("공연 한 건의 발권과 배분을 담당하는 계약이다. 우선권·일반 트랙으로 나누어 응모를 받고(enterDraw) 추첨을 확정하며(draw), 공식 반납 창구가 아니면 어떤 전송도 막는 transferFrom을 두었다. 표를 반납하면 대금을 전액 돌려주고(returnTicket), 현장 검표 시 여권에 기록을 남기며(checkIn), 무단 노쇼는 환불하지 않는다(closeNoShow). 다만 현재 코드는 노쇼 억제 장치로 별도의 보증금을 예치·몰수하는 방식을 쓰고 있어, 본문에서 제시한 '표값 자체를 담보로 삼는' 단순화된 규칙은 후속 반영 대상이다.")]),
